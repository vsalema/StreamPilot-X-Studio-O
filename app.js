...
function buildRow(ch){
  ...
  // 1) Si la CustomList est ouverte, on la "met en pause" en la fermant via son bouton
  try {
    const btnToggleCustom = document.getElementById('btnToggleCustomList');
    if (btnToggleCustom && btnToggleCustom.getAttribute('aria-pressed') === 'true') {
      // On ferme proprement en utilisant sa propre logique interne
      btnToggleCustom.click();
    }
  } catch (e) {
    console.warn('Erreur lors de la désactivation de CustomList', e);
  }

  // 2) Sélection de la chaîne dans channelList (comportement existant)
  const idx = channelsData.findIndex(x => x.url === ch.url && x.name === ch.name);
  playChannelAt(idx >= 0 ? idx : 0);

  // 3) Sur mobile, on referme l’aside comme avant
  closeAsideMobile();
});

...
function populateChannels(channels){
  ...
  // 🔄 Vérification automatique après construction de la liste
  if (typeof verifyVisibleLinks === 'function') {
    verifyVisibleLinks().catch(err => console.warn('verifyVisibleLinks (auto) error:', err));
  }
}
...
if (btnCheckLinks) {
  btnCheckLinks.addEventListener('click', verifyVisibleLinks);
  // Optionnel : texte / tooltip pour montrer que c’est une relance
  btnCheckLinks.title = 'Relancer la vérification des liens';
}
...
(function(){
  ...
  async function loadM3UFast(url){
    ...
    // 🔄 Vérification automatique aussi après chargement FAST
    if (typeof window.verifyVisibleLinks === 'function') {
      window.verifyVisibleLinks().catch(err => console.warn('verifyVisibleLinks (fast-auto) error:', err));
    }
